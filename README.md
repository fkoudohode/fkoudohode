# Florent Koudohode | Personal academic website
...
├── work.html               ← Page double : chooser between Research / Publications
...
│
├── README.md               ← This file
│  (no CNAME — the site publishes on the default github.io URL.
│   Recreate CNAME later if you decide to use a custom domain.)
│
└── assets/
    ├── img/
    │   ├── profile-photo.jpg     ← Portrait 1200×1500 (page About)
    │   ├── profile-avatar.jpg    ← Square 400×400 (hero card)
    │   ├── profile-brand.png     ← Circular 128×128 (navbar logo)
    │   └── gallery/              ← Optional gallery images
    ├── papers/                   ← PDFs of articles
    ├── code/                     ← Code archives / reproducibility material
    ├── docs/
    │   ├── cv-koudohode-en.pdf   ← English CV
...
```js
{
  id:       "J6",                          // unique key — used as anchor #J6
  type:     "journal",                     // "journal" | "conference" | "preprint" | "thesis"
  status:   "published",                   // "published" | "accepted" | "under-review" | "preprint"
  year:     2026,
  title:    "Your paper title here",
  authors:  ["F. Koudohode", "Co-Author"], // your name is highlighted automatically
  venue:    "Automatica",
  venueDetail: "vol. 178, 110627",         // optional
  ...
  code:     "https://github.com/fkoudohode/J6-code", // optional — link to your repo
  codeLabel:"MATLAB",                      // optional — adds "MATLAB" or "Python" badge
  abstract: "Optional plain-text abstract…"
}
```

**Pour les liens de code MATLAB / Python** : mettez l'URL GitHub dans `code:` et précisez `codeLabel: "MATLAB"` ou `"Python"`. Le bouton apparaîtra automatiquement sur la même ligne que BibTeX et DOI.
...
1. Créez un repo public nommé **`VOTRE-USERNAME.github.io`** (par ex. `fkoudohode.github.io`).
2. Poussez le contenu de `site/` à la racine du repo.
3. Settings → Pages → Source : `main` / `(root)` → Save.
4. Votre site est en ligne à `https://VOTRE-USERNAME.github.io/` au bout de 1-2 minutes.

> **Pas de domaine personnalisé pour l'instant** — le fichier `CNAME` a été retiré. Quand vous voudrez utiliser `fkoudohode.com` (ou un autre domaine), il suffira :
>  1. de recréer un fichier `CNAME` à la racine contenant simplement `fkoudohode.com`,
>  2. de configurer les enregistrements DNS chez votre registrar (voir la procédure que je vous ai donnée pour Squarespace),
>  3. de saisir le domaine dans GitHub → Settings → Pages → Custom domain.
...
`scp -r site/ user@server:/var/www/yoursite.com/` and you're done.
